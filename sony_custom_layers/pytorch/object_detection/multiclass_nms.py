...
def multi_class_nms(boxes: Tensor, scores: Tensor, score_threshold: float, iou_threshold: float,
                    max_detections: int) -> NMSResults:
    """
    Performs multi-class non-maximum suppression on a batch of images.
    Detections are returned in decreasing scores order.
    Output tensors always contain max_detections detections, possibly with zero-padding.
    ...
    Args:
        ...
        score_threshold: score threshold for boxes selection. Boxes with score below threshold are discarded (per label)
        iou_threshold: intersection over union threshold for suppression of similar boxes
        max_detections: constant number of detections to return.
    ...
    batch = boxes.shape[0]
    res = [
        _image_multi_class_nms(boxes[i],
                               scores[i],
...
def _image_multi_class_nms(boxes: Tensor, scores: Tensor, score_threshold: float, iou_threshold: float,
                           max_detections: int) -> NMSResults:
    """
    Performs multi-class non-maximum suppression on a single image
    Same interface as multi_class_nms, only without the batch dimension.
    """
